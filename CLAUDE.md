...
**В начале сессии** → прочитай `SESSION.md` и `TODO.md` для контекста.
...
- Аналитика / новые кликабельные элементы → прочитай `docs/ANALYTICS.md`, используй PostHog MCP плагин (`/posthog:insights`)
...
- После коммита → **перенеси** текущий `SESSION.md` в начало `SESSION-log.md`, затем **перезапиши** `SESSION.md` (что сделали, что дальше)
...
Тег `AW-16731693756` **инжектируется автоматически в `scripts/build.js`** (шаг 18) на все страницы при сборке. Не добавляй его вручную в HTML-файлы источников.

Аналогично инжектируется PostHog (шаг 19).

При создании **новой страницы** — добавь её в `PAGES` в `build.js`, и теги подтянутся автоматически.
